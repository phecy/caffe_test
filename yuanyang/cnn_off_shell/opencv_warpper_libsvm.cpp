...
bool opencv_warpper_libsvm::save_model( const string &path_of_model,
                         const string &path_of_info) const
{
    if( svm_save_model( path_of_model.c_str(), m_model)!= 0 )
    {
        cout<<"Can not save the model to file "<<path_of_model<<endl;
        return false;
    }
    
    FileStorage fs( path_of_info, FileStorage::WRITE);
    if( !fs.isOpened())
    {
        cout<<"Can not open "<<path_of_info<<" for writing the file"<<endl;
        return false;
    }
    fs<<"matrix"<<m_feature_dim;
    fs.release();
    return true;
}

bool opencv_warpper_libsvm::load_model( const string &model_path,
                                        const string &model_info)
{
    /*  load svm model */
    m_model = svm_load_model( model_path.c_str());
    if( !m_model )
    {
        cout<<"Load model from "<<model_path<<" failed, check the file please"<<endl;
        return false;
    }

    /* load feature infos */
    FileStorage fs(model_info, FileStorage::READ);
    if( !fs.isOpened())
    {
        cout<<"Can not open file "<<model_info<<endl;
        return false;
    }
    fs["matrix"]>>m_feature_dim;
